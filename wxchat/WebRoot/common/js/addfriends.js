...
$(function (){
	...
	function addclose(){
	 ...
	 var obj;
     function selFriend(inp){
   	  
   	  var val = inp.value;
   	  
   	  var data = "account="+val+"&oper=sel";
   	  myAjax("post","friend",data,true,"application/x-www-form-urlencoded",deal);
     }
     var deal = function(ajax){
    	 ...
    	 var result = ajax.responseText;
			...
			if(obj){
				...
				$("#headImg").attr("src","headImg/"+$("#so").val()+".jpg");
				$("#headImg").css("height","100%");
				$("#user").html(obj.nickname);
				$("#circle").attr("src","common/img/circle010.png");
			}else{
				alert("未找到该用户");
			}
     ...
     function addFriend(){
   	  var myAccount = $("#hid").val();
   	 	
   	  var data = "account="+myAccount+"&oper=add"+"&faccount="+obj.account;
   	  if($('#user').html()!=""||$('#user').html()!=null){
   		  
   		 myAjax("post","friend",data,true,"application/x-www-form-urlencoded",deal200,deal404);
   	  }
